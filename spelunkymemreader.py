#most of this is straight up stolen from https://stackoverflow.com/questions/44901892/python-how-to-get-the-value-at-a-memory-address
import ctypes
...
#this little bit is by Kirby703
#TODO: refactor to make readWord not open+close the handle with literally every call
def readWord(pid, address):
    buf = ctypes.create_string_buffer(b'abcd')
    size = len(buf.value)
    value = read_process_memory(pid, address, size)
    return value[3]*2**24 + value[2]*2**16 + value[1]*2**8 + value[0]
